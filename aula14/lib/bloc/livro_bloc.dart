...
class LivroBloc {
  ...
  LivroBloc({required this.context, required this.editora, this.livro}) {
    if (livro != null) {
      ...
      precoController.text = livro!.toString();
    }
  }
  ...
  void salvar() async {
    if (nomeController.text.isEmpty) {
      MensagemAlerta().show(context: context, titulo: "Atenção", texto: "O nome do livro é obrigatório");
      return;
    }

    if (precoController.text.toDouble(vlrPadrao: -1) < 0) {
      MensagemAlerta().show(context: context, titulo: "Atenção", texto: "O valor do livro é obrigatório");
      return;
    }
  ...
  void excluir({int? codigo}) {
    MensagemPopup().show(
      ...
      botoes: [
        BotaoTexto(texto: "Sim", clique: () { _confirmarExcluir(codigo: codigo); }, cor: Colors.red),
  ...
  void _confirmarExcluir({int? codigo}) async {
    if (livro != null) {
      _livroHelper.excluir(livro!.codigo ?? 0);
      Navigator.pop(context);
    }
    else if (codigo != null) {

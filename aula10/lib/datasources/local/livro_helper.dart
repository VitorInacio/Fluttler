...
class LivroHelper {
  static const createSql = '''
    CREATE TABLE IF NOT EXISTS TbLivro (
      ...
      preco REAL,
      editora INTEGER,
      FOREIGN KEW editora REFERENCES TbEditora(codigo)
    )
  ''';
  ...
  Future<int> alterar(Livro livro) async {
    ...
    int retorno = await db.update('TbLivro', livro.toMap(), where: 'codigo = ?', whereArgs: [livro.codigo]);
  ...
  Future<int> excluir(int codigo) async {
    ...
    int retorno = await db.delete('TbLivro', where: 'codigo = ?', whereArgs: [codigo]);
